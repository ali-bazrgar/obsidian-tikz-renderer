...
if (!processor.includes("const editLink = `[✎ Edit TikZ](#tikz-edit:${historyKey})`;")) throw new Error("Writing mode must expose a generated TikZ edit link.");
if (!processor.includes("GENERATED_EDIT_CLASS")) throw new Error("Generated TikZ edit links must be identifiable independently from the SVG link.");
...
if (!compactCss.includes('.tikz-renderer-viewport{position:relative') || !compactCss.includes('border:0')) throw new Error("The inner TikZ viewport must not create a second visual window.");
if (!css.includes('.markdown-preview-view .tikz-renderer-controls,.markdown-preview-view .tikz-renderer-panel{display:none!important}')) throw new Error("TikZ controls and settings must be hidden in Reading view.");
...
if (!css.includes('.markdown-preview-view .tikz-generated-edit-link{display:none!important}')) throw new Error("Generated TikZ edit links must be hidden in Reading view.");
...
console.log("Verified: inline SVG, hidden popup startup, capture-phase Ctrl+wheel zoom, one visual figure layer, left-attached controls, non-clickable figure, Writing-only edit controls, and Reading-view-only generated links.");
