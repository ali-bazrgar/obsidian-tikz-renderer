...
export class TikzRendererView {
  ...
  private static readonly activeViews = new WeakMap<HTMLElement, TikzRendererView>();
  ...
  render(): void {
    // A Markdown processor can recreate a renderer for the same host. Dispose
    // the previous portal before creating another one, otherwise fixed panels
    // remain at the top-left of the Obsidian window.
    TikzRendererView.activeViews.get(this.host)?.dispose();
    TikzRendererView.activeViews.set(this.host, this);
    this.dispose();
    this.host.empty();
    ...
    // Controls and panel are deliberately portaled to <body>. They never become
    // descendants of the figure, so figure overflow/size cannot clip them.
    ...
    const positionPanel = (): void => {
      ...
      const width = Math.max(190, panelRect.width);
    ...
    const applyZoom = (): void => {
      // Do not use transform:scale() for zoom. The image's actual dimensions
      // change, so the Markdown layout follows the user's selected size and the
      // background paper is exactly the same size as the rendered figure.
    ...
    // Close on a pointer press anywhere outside the detached controls/panel.
    // Capture phase is intentional: no child handler can prevent this from
    // seeing an outside click.
    ...
    body.addEventListener("pointerdown", outsidePointerDown, true);
    ...
    const renderThemeMenu = (): void => {
      ...
      for (const [theme, label] of themes) {
        const button = panel.createEl("button", {
          ...
          attr: {
            type: "button",
            role: "menuitemradio",
            "aria-checked": `${this.getSettings().displayTheme === theme}`,
          },
        });
      ...
      if (this.getSettings().displayTheme === "custom") {
        const color = panel.createEl("input", {
          attr: {
            type: "color",
            value: this.getSettings().customBackgroundColor,
        ...
        const opacity = panel.createEl("input", {
          attr: {
            ...
            max: "100",
            step: "1",
            value: `${this.getSettings().customBackgroundOpacity}`,
    ...
    assetLink.addEventListener("click", (event) => {
      ...
      event.preventDefault();
      event.stopPropagation();
      if (zoom <= 1 && this.result.assetPath) {
    ...
    // Ctrl/Cmd + wheel changes the actual width/height of the image. Ordinary
    // wheel events are untouched, preserving normal Obsidian scrolling.
    ...
    this.cleanup = () => {
      body.removeEventListener("pointerdown", outsidePointerDown, true);
      ...
      controls.remove();
      panel.remove();
      if (TikzRendererView.activeViews.get(this.host) === this) {
        TikzRendererView.activeViews.delete(this.host);
      }
  ...
  private showHistory(panel: HTMLElement): void {
    ...
    entries.forEach((entry, index) => {
      ...
      if (entry.source !== this.source) {
        row.createEl("button", { text: "Restore", attr: { type: "button" } })
          .addEventListener("click", () => new TikzSourceModal(this.app, entry.source, async (next) => this.editSource(next)).open());
      }
    });
  }
...
class TikzSourceModal extends Modal {
  constructor(
    ...
    private readonly onRender: (source: string) => Promise<void>,
  ) {
    super(app);
  }
  ...
  onOpen(): void {
    ...
    actions.createEl("button", { text: "Render", attr: { type: "button" } })
