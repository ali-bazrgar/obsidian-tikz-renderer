...
export class TikzRendererView extends MarkdownRenderChild {
  ...
  render(): void {
    ...
    const initialEditState: TikzViewState = persisted ?? { zoom: clampZoom(settings.defaultZoom / 100), panX: 0, panY: 0 };
    TikzRendererView.viewStates.set(stateKey, initialEditState);
    let zoom = initialEditState.zoom, panX = initialEditState.panX, panY = initialEditState.panY;
    ...
    const persistEditViewState = (): void => { if (isReadingMode()) return; const state = { zoom, panX, panY }; TikzRendererView.viewStates.set(stateKey, state); saveViewState(stateKey, state); for (const view of TikzRendererView.allViews) { if (view !== this && view.sharedStateKey === stateKey) view.applyExternalState?.(state); } };
    ...
    const resetView = (): void => { if (isReadingMode()) return; zoom = clampZoom(settings.defaultZoom / 100); panX = 0; panY = 0; persistEditViewState(); applyZoom(); };
    const fit = (): void => { if (isReadingMode() || !ensureIntrinsicSize()) return; const width = Math.max(1, this.containerEl.clientWidth); zoom = clampZoom(Math.min(1, width / naturalWidth)); panX = 0; panY = 0; persistEditViewState(); applyZoom(); };
    ...
    const stopDragging = (): void => { if (!dragging) return; dragging = false; if (!isReadingMode()) persistEditViewState(); viewport.classList.remove("is-dragging"); };
