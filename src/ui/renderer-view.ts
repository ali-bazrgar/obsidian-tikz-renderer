...
export class TikzRendererView extends MarkdownRenderChild {
  ...
  render(): void {
    ...
    let zoom = clampZoom(persisted?.zoom ?? settings.defaultZoom / 100), panX = persisted?.panX ?? 0, panY = persisted?.panY ?? 0;
    let naturalWidth = 0, naturalHeight = 0, viewportHeight = 0, dragging = false, lastX = 0, lastY = 0;
    ...
    const applyLocalViewState = (state: TikzViewState): void => { zoom = clampZoom(state.zoom); panX = state.panX; panY = state.panY; if (naturalWidth > 0 && naturalHeight > 0) { svg.style.width = `${naturalWidth * zoom}px`; svg.style.height = `${naturalHeight * zoom}px`; svg.style.maxWidth = "none"; } svg.style.transform = `translate3d(${panX}px, ${panY}px, 0)`; viewport.classList.toggle("is-pannable", zoom > 1); viewport.style.touchAction = "none"; };
    const syncSharedState = (): boolean => { const shared = TikzRendererView.viewStates.get(stateKey); if (!shared) return false; const changed = zoom !== shared.zoom || panX !== shared.panX || panY !== shared.panY; if (changed) applyLocalViewState(shared); return changed; };
    const updateMode = (): boolean => { const reading = isReadingMode(); const stateChanged = syncSharedState(); shell.dataset.mode = reading ? "reading" : "writing"; controls.hidden = reading; if (reading) { dragging = false; closePanel(); } return stateChanged; };
    const persistViewState = (): void => { const state = { zoom, panX, panY }; TikzRendererView.viewStates.set(stateKey, state); saveViewState(stateKey, state); for (const view of TikzRendererView.allViews) { if (view !== this && view.sharedStateKey === stateKey) view.applyExternalState?.(state); } };
    ...
    const ensureViewportSize = (): void => { if (!ensureIntrinsicSize()) return; const baseZoom = clampZoom(settings.defaultZoom / 100); const width = Math.max(1, this.containerEl.clientWidth || naturalWidth * baseZoom); viewportHeight = Math.max(1, naturalHeight * baseZoom); viewport.style.width = `${width}px`; viewport.style.height = `${viewportHeight}px`; viewport.style.maxWidth = "100%"; };
    const applyZoom = (): void => { ensureViewportSize(); if (naturalWidth > 0 && naturalHeight > 0) { svg.style.width = `${naturalWidth * zoom}px`; svg.style.height = `${naturalHeight * zoom}px`; svg.style.maxWidth = "none"; } svg.style.transform = `translate3d(${panX}px, ${panY}px, 0)`; viewport.classList.toggle("is-pannable", zoom > 1 || isReadingMode()); viewport.classList.toggle("is-dragging", dragging); viewport.style.touchAction = "none"; applyTheme(); updateMode(); positionPanel(); };
    const resetView = (): void => { if (isReadingMode()) return; zoom = clampZoom(settings.defaultZoom / 100); panX = 0; panY = 0; persistViewState(); applyZoom(); };
    const fit = (): void => { if (isReadingMode() || !ensureIntrinsicSize()) return; const width = Math.max(1, this.containerEl.clientWidth); zoom = clampZoom(Math.min(1, width / naturalWidth)); panX = 0; panY = 0; persistViewState(); applyZoom(); };
    ...
    viewport.addEventListener("pointerdown", e => { if (e.button !== 0 || (!isReadingMode() && zoom <= 1)) return; dragging = true; lastX = e.clientX; lastY = e.clientY; viewport.setPointerCapture(e.pointerId); viewport.classList.add("is-dragging"); e.preventDefault(); });
    viewport.addEventListener("pointermove", e => { if (!dragging) return; panX += e.clientX - lastX; panY += e.clientY - lastY; lastX = e.clientX; lastY = e.clientY; svg.style.transform = `translate3d(${panX}px, ${panY}px, 0)`; e.preventDefault(); });
    const stopDragging = (): void => { if (!dragging) return; dragging = false; persistViewState(); viewport.classList.remove("is-dragging"); }; viewport.addEventListener("pointerup", stopDragging); viewport.addEventListener("pointercancel", stopDragging); viewport.addEventListener("lostpointercapture", stopDragging);
    const wheel = (e: WheelEvent): void => { const reading = isReadingMode(); if ((!reading && (!e.ctrlKey && !e.metaKey)) || !shell.isConnected) return; const target = e.target; if (!(target instanceof Node) || !viewport.contains(target)) return; if (target instanceof HTMLInputElement || target instanceof HTMLButtonElement) return; if (!ensureIntrinsicSize()) return; e.preventDefault(); e.stopPropagation(); const old = zoom; const next = clampZoom(old * (e.deltaY < 0 ? 1.12 : 1 / 1.12)); if (next === old) return; const r = viewport.getBoundingClientRect(); const x = e.clientX - r.left; const y = e.clientY - r.top; const ratio = next / old; panX = x - (x - panX) * ratio; panY = y - (y - panY) * ratio; zoom = next; persistViewState(); applyZoom(); };
...
function loadViewState(key: string): TikzViewState | undefined { try { const raw = window.localStorage.getItem(viewStorageKey(key)); if (!raw) return undefined; const p = JSON.parse(raw) as Partial<TikzViewState>; if (!Number.isFinite(p.zoom) || !Number.isFinite(p.panX) || !Number.isFinite(p.panY)) return undefined; return { zoom: clampZoom(Number(p.zoom)), panX: Number(p.panX), panY: Number(p.panY) }; } catch { return undefined; } }
