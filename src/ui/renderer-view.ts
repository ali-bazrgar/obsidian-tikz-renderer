...
export class TikzRendererView extends MarkdownRenderChild {
  ...
  render(): void {
    ...
    viewport.addEventListener("pointerdown", e => { if (isReadingMode() || e.button !== 0 || !canPan()) return; dragging = true; lastX = e.clientX; lastY = e.clientY; viewport.setPointerCapture(e.pointerId); viewport.classList.add("is-dragging"); e.preventDefault(); });
    ...
    const wheel = (e: WheelEvent): void => { if (isReadingMode() || !shell.isConnected) return; if (!e.ctrlKey && !e.metaKey) return; const target = e.target; if (!(target instanceof Node) || !viewport.contains(target)) return; if (target instanceof HTMLInputElement || target instanceof HTMLButtonElement) return; if (!ensureIntrinsicSize()) return; e.preventDefault(); e.stopPropagation(); const old = zoom; const factor = e.deltaY < 0 ? 1.12 : 1 / 1.12; const next = clampZoom(old * factor); const r = viewport.getBoundingClientRect(); const x = e.clientX - r.left; const y = e.clientY - r.top; const ratio = next / old; if (next !== old) { panX = x - (x - panX) * ratio; panY = y - (y - panY) * ratio; zoom = next; } clampPan(); persistEditViewState(); applyZoom(); };
