...
export class TikzMarkdownProcessor {
  static async process(app: App, exportService: ExportService, history: TikzHistoryStore, kind: BlockKind, source: string, el: HTMLElement, ctx: MarkdownPostProcessorContext, service: RenderService, getSettings: () => TikzSettings, saveSettings: (settings: TikzSettings) => Promise<void>): Promise<void> {
    ...
    try {
      ...
      const view = new TikzRendererView(app, exportService, host, result, source, ctx.sourcePath, service, kind, history, historyKey, async nextSource => replaceSource(app, ctx, el, kind, nextSource), getSettings, saveSettings, async () => confirmAssetLink(app, kind), async assetPath => { if (section) await ensureSourceAssetLinks(app, ctx.sourcePath, section, assetPath, historyKey, kind); });
      ctx.addChild(view);
      view.render();
