...
class TikzSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(container).setName("Engine").addDropdown((dropdown) => dropdown.addOptions({ auto: "Auto", latex: "latex", pdflatex: "pdflatex", xelatex: "xelatex", lualatex: "lualatex", dvilualatex: "dvilualatex" }).setValue(this.plugin.settings.engine).onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, engine: value as TikzSettings["engine"] }); }));
    new Setting(container).setName("TeX Live root").setDesc("Accepts either the installation root or its Windows binary directory.").addText((text) => text.setPlaceholder("D:\\texlive\\2025").setValue(this.plugin.settings.texLiveRoot).onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, texLiveRoot: value.trim() }); }));
    ...
    for (const [key, name] of paths) new Setting(container).setName(name).addText((text) => text.setValue(String(this.plugin.settings[key])).onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, [key]: value.trim() }); }));
    new Setting(container).setName("Asset folder").addText((text) => text.setValue(this.plugin.settings.assetFolder).onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, assetFolder: normalizePath(value.trim()) }); }));
    new Setting(container).setName("Cache folder").addText((text) => text.setValue(this.plugin.settings.cacheFolder).onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, cacheFolder: normalizePath(value.trim()) }); }));
    new Setting(container).setName("Compile timeout (ms)").setDesc("Maximum time for one TeX/vector conversion process.").addText((text) => text.setValue(String(this.plugin.settings.compileTimeout)).onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, compileTimeout: Math.max(5000, Math.min(120000, Number(value) || 120000)) }); }));
    new Setting(container).setName("Default zoom").addSlider((slider) => slider.setLimits(25, 500, 25).setValue(this.plugin.settings.defaultZoom).setDynamicTooltip().onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, defaultZoom: value }); }));
    new Setting(container).setName("Display theme").setDesc("Changes the figure background only; TikZ source and SVG colors are never rewritten.").addDropdown((dropdown) => dropdown.addOptions({ auto: "Auto", obsidian: "Obsidian", light: "Light", paper: "Paper", dark: "Dark", contrast: "Contrast", bw: "Black & white", custom: "Custom" }).setValue(this.plugin.settings.displayTheme).onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, displayTheme: value as TikzSettings["displayTheme"] }); }));
    new Setting(container).setName("Custom background").addColorPicker((picker) => picker.setValue(this.plugin.settings.customBackgroundColor).onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, customBackgroundColor: value }); }));
    new Setting(container).setName("Custom background opacity").addSlider((slider) => slider.setLimits(10, 100, 1).setValue(this.plugin.settings.customBackgroundOpacity).setDynamicTooltip().onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, customBackgroundOpacity: value }); }));
    new Setting(container).setName("Keep TeX source").addToggle((toggle) => toggle.setValue(this.plugin.settings.keepTexSource).onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, keepTexSource: value }); }));
    new Setting(container).setName("Persian font").addText((text) => text.setValue(this.plugin.settings.persianFont).onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, persianFont: value.trim() }); }));
    new Setting(container).setName("History limit").addSlider((slider) => slider.setLimits(1, 100, 1).setValue(this.plugin.settings.historyLimit).setDynamicTooltip().onChange(async (value) => { await this.plugin.saveSettings({ ...this.plugin.settings, historyLimit: value }); }));
