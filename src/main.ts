...
export default class TikzRendererPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.addCommand({
      id: "detect-tex-executables",
      name: "Detect TeX Live executables",
      callback: async () => {
        await this.saveSettings({ ...this.settings, ...(await this.renderService.detectExecutables()) });
        new Notice("TeX executable detection complete.");
      },
    });

    this.addCommand({
      id: "clear-render-cache",
      name: "Clear TikZ render cache",
      callback: async () => {
...
class TikzSettingTab extends PluginSettingTab {
  constructor(app: App, private readonly plugin: TikzRendererPlugin) {
    super(app, plugin);
  }
  ...
  display(): void {
    ...
    const paths: Array<[keyof TikzSettings, string]> = [
      ...
      ["lualatexPath", "lualatex path"], ["dvilualatexPath", "dvilualatex path"], ["dvisvgmPath", "dvisvgm path"],
      ["mutoolPath", "mutool path"],
    ];
    for (const [key, name] of paths) {
      new Setting(container).setName(name).addText((text) => text
        .setValue(String(this.plugin.settings[key]))
        .onChange(async (value) => this.plugin.saveSettings({ ...this.plugin.settings, [key]: value.trim() })));
    }

    new Setting(container).setName("Asset folder").addText((text) => text
    ...
    new Setting(container).setName("Cache folder").addText((text) => text
    ...
    new Setting(container).setName("Compile timeout (ms)").addText((text) => text
    ...
    new Setting(container).setName("Default zoom").addSlider((slider) => slider
      .setLimits(25, 500, 25).setValue(this.plugin.settings.defaultZoom).setDynamicTooltip()
    ...
    new Setting(container).setName("Keep TeX source").addToggle((toggle) => toggle
      .setValue(this.plugin.settings.keepTexSource)
      .onChange(async (value) => this.plugin.saveSettings({ ...this.plugin.settings, keepTexSource: value })));

    new Setting(container).setName("Persian font").addText((text) => text
      .setValue(this.plugin.settings.persianFont)
      .onChange(async (value) => this.plugin.saveSettings({ ...this.plugin.settings, persianFont: value.trim() })));

    new Setting(container).setName("History limit").addSlider((slider) => slider
      .setLimits(1, 100, 1).setValue(this.plugin.settings.historyLimit).setDynamicTooltip()
