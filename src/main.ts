...
class TikzSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(container).setName("Persian font").setDesc("Exact XeLaTeX/fontconfig family name, for example Vazirmatn. The renderer validates it before Persian rendering.").addText((text) => text.setValue(this.plugin.settings.persianFont).onChange(async (value) => this.plugin.saveSettings({ ...this.plugin.settings, persianFont: value.trim() })));
