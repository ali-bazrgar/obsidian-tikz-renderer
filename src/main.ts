...
class TikzSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(container).setName("Best-effort output").setDesc("Convert a valid PDF/DVI/XDV artifact even when TeX exits non-zero. The artifact must be produced by the current run.").addToggle((toggle) => toggle.setValue(this.plugin.settings.bestEffortOutput).onChange(async (value) => this.plugin.saveSettings({ ...this.plugin.settings, bestEffortOutput: value })));
    new Setting(container).setName("Shell escape").setDesc("Disabled is safest. Enable only for documents that explicitly require external commands.").addDropdown((dropdown) => dropdown.addOptions({ disabled: "Disabled", restricted: "Restricted", enabled: "Enabled" }).setValue(this.plugin.settings.shellEscape).onChange(async (value) => this.plugin.saveSettings({ ...this.plugin.settings, shellEscape: value as TikzSettings["shellEscape"] })));
    ...
    new Setting(container).setName("Persian font family").setDesc("Exact font family recognized by XeLaTeX/fontconfig, for example Vazirmatn. Used when no explicit font file path is set.").addText((text) => text.setPlaceholder("Vazirmatn").setValue(this.plugin.settings.persianFont).onChange(async (value) => this.plugin.saveSettings({ ...this.plugin.settings, persianFont: value.trim() })));
    new Setting(container).setName("Persian font file").setDesc("Optional full path to a .ttf, .otf or .ttc file. When set, this takes precedence over the family name.").addText((text) => text.setPlaceholder("C:\\Fonts\\Vazirmatn-Regular.ttf").setValue(this.plugin.settings.persianFontPath).onChange(async (value) => this.plugin.saveSettings({ ...this.plugin.settings, persianFontPath: value.trim() })));
    new Setting(container).setName("Test Persian font").setDesc("Compiles a tiny XeLaTeX probe using the current font settings.").addButton((button) => button.setButtonText("Test").onClick(async () => { const result = await this.plugin.renderService.testPersianFont(); new Notice(result.message, result.ok ? 5000 : 10000); }));
