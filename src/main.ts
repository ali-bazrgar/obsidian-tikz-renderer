...
class TikzSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(container).setName("Display theme").addDropdown((dropdown) => dropdown.addOptions({ auto: "Auto", obsidian: "Obsidian", light: "Light", paper: "Paper", dark: "Dark", contrast: "Contrast", bw: "Black & White" }).setValue(this.plugin.settings.displayTheme).onChange(async (value) => this.plugin.saveSettings({ ...this.plugin.settings, displayTheme: value as TikzSettings["displayTheme"] })));
